...
&gt; 版本：v2.2 | 状态：✅已完成 | 最后更新：2026-05-21
...
├────────────────────────────────────┤
│  ┌──────────────────────────────┐  │
│  │ 💵 现金 [默认支出][默认收入] │  │ ← WdSwipeCell 左滑区域
│  │     ¥3,210.00 日常现金备用    │  │   badge标识默认账户
│  └──────────────────────────────┘  │
│  ┌──────┐  ┌──────┐              │
...
- **图标**：左侧显示账户图标（emoji）
- **名称**：账户名称（加粗）
- **默认账户标识**：名称右侧显示badge标签
  - `[默认支出]`：蓝色渐变背景（#00BFFF → #0099CC），白色文字
  - `[默认收入]`：绿色渐变背景（#19BE6B → #14A85D），白色文字
  - 两个标签可同时显示（同一账户既是默认支出又是默认收入）
  - 非默认账户不显示标签
- **说明**：账户说明（可选，灰色小字）
- **余额**：右侧显示余额（资产绿色，负债红色）
- **左滑操作区**：
  - 默认支出/收入账户：仅显示编辑按钮（主色）
